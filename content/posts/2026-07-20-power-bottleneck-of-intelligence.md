...
글로벌 데이터센터 전력 소비는 2025년 485TWh에서 2030년 약 950TWh로 두 배 가까이 폭증하며 전 세계 전력의 약 3%를 차지할 전망이다. 특히 AI 특화 데이터센터의 전력 소비는 같은 기간 3배나 늘어난다.
...
한편, 글로벌 인프라 대격변 속에서 한국은 극단적인 양면을 동시에 맞닥뜨렸다. 전력이 병목이 되면 자연스레 전력 대비 성능(전성비)이 핵심 경쟁력이 되는데, 그 싸움의 중심에 있는 HBM이 한국의 확실한 수혜처다.
...
하지만 치명적인 리스크는 역시 전력망이다. 삼성전자 15GW, SK하이닉스 6.3GW를 합쳐 원전 10기가 넘는 전력을 용인 반도체 클러스터로 끌어와야 한다. 초고압 송전망과 변전소 건설에 통상 10년 이상이 걸린다는 점을 감안하면 참으로 역설적이구나 싶다.

수도권 데이터센터의 전력 공급 승인률은 1.9%에 불과한데, 정작 민간 데이터센터의 73.4%가 여전히 수도권에 밀집해 있다.
