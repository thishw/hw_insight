...
AI 개발의 역사는 지금까지 하나의 공식이 지배해 왔다. 더 많은 데이터, 더 많은 파라미터, 더 많은 컴퓨팅 — 이른바 <strong>사전학습 스케일링 법칙(pre-training scaling law)</strong>이다.
...
개인적으로 이 계보에서 중요한 건 명칭이 아니라 방향이라고 본다. AI 개발의 무게중심이 <strong>모델을 만드는 곳(사전학습)</strong>에서 <strong>모델을 운용하는 방식(추론-시간 스케일링과 에이전틱 설계)</strong>으로 옮겨가고 있다는 신호다.
