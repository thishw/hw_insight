...
며칠 전, 프롬프트 한 줄을 넣고 자리를 비웠다가 돌아왔더니 **게임 하나가 만들어져 있었다.**

코드 조각이 아니라, 규칙이 돌아가고 충돌 판정이 되고 화면이 움직이는 '완성품'이었다. 예전 같으면 "이 함수 리팩터링해줘", "이 데이터 분석해줘" 정도의 작업 단위를 시켰을 텐데, 이번엔 결과물이 통째로 나와버렸다. 물론 기대한 만큼의 퀄리티는 아니었다. 그런데 **"일단 프로덕트까지는 나와버린다"**는 사실 자체가 충격이었다.

이번에 나온 [Claude Fable 5](https://www.anthropic.com/news/claude-fable-5-mythos-5)를 쓰면서 든 생각이다. 개인적으로는 이게 단순한 "더 똑똑한 모델 하나 나왔다"가 아니라, **AI 시장의 국면이 바뀌는 신호**로 보여서 정리해 둔다.

---

> **핵심 요약**
> AI는 채팅으로 정보를 정리해 주던 1세대에서, 사람의 업무를 대신하는 2세대를 지나, 이제 **서비스·프로덕트 자체를 통째로 만들어 내는 3세대**로 넘어가고 있다. 그 임계점을 현존 최고 성능 모델 Fable 5가 보여줬다. 그런데 이 지능은 너무 비싸고 귀해서 정액 구독에 담기지 못하고, 며칠짜리 이벤트가 끝나면 토큰당 과금($10/$50 per 100만 토큰)으로 밀려난다. 반대편엔 100만 토큰에 몇 센트 하는 저가 중국 모델이 있다. 하나는 **소매 완제품 지능**, 다른 하나는 하네스로 조립해 쓰는 **도매 원료 지능**이다. 지능이 계층별로 값이 매겨지는 이 분화가, AI 시장이 진짜 확장기에 들어섰다는 신호로 보인다. 그리고 그 끝에는 오래된 명제 하나가 있다 — 똑똑함이 흥망성쇠를 가른다.
...
그래서 나는 지금 AI 시장이 **도매(卸賣)와 소매(小賣)로 갈라지고 있다**고 본다.

가격표의 양 끝을 보자. 위쪽 끝에 Fable이 있고, 아래쪽 끝에 DeepSeek 같은 저가 모델이 있다. 출력 토큰 기준으로 **Fable은 DeepSeek의 약 180배**다.
...
<figcaption>같은 '토큰'인데 값은 하늘과 땅이다. 맨 왼쪽 DeepSeek 막대는 거의 보이지 않는데, 그게 바로 '도매 원료'의 위치다.</figcaption>

이 격차는 성능 격차이기도 하지만, **'무엇을 파느냐'의 차이**이기도 하다.

- **최선단 모델(Fable)은 소매 완제품을 판다.** 프롬프트 한 번 넣으면 쓸 만한 결과가 통째로 나온다. 지능을 '이미 조립된 상태'로 받는다. 비싼 이유다.
- **저가 모델(DeepSeek 등)은 도매 원료를 판다.** 그 자체로는 살짝 부족하지만, 아주 싸다. 대신 쓸 만한 결과를 얻으려면 사람이 **한땀한땀 하네스를 짜서** 소매 완제품으로 조립해야 한다.

```mermaid
flowchart LR
    W["저가 모델<br/>(도매 원료)<br/>DeepSeek 등"] -->|"하네스·패키징<br/>한땀한땀 조립"| P["쓸 만한 프로덕트<br/>= 소매 지능"]
    F["최선단 모델<br/>(소매 완제품)<br/>Fable"] -->|"프롬프트 한 번"| P
    style F fill:#4F46E5,color:#fff
    style P fill:#059669,color:#fff
```

<figcaption>같은 목적지(쓸 만한 프로덕트)에 도달하는 두 경로. 완제품을 비싸게 사거나, 원료를 싸게 사서 직접 조립하거나.</figcaption>

여기서 재미있는 가설이 나온다. **저가 모델을 아주 잘 패키징·하네스 엔지니어링하면, Fable과 비슷한 모습이 되지 않을까?** 원료가 조금 부족해도, 컨텍스트를 잘 쌓고 루프를 잘 설계하고 검증을 잘 붙이면, 도매 원료로도 꽤 괜찮은 소매 완제품을 조립해 낼 수 있다. 실제로 지난 몇 년간 사람들이 컴파운드 AI 시스템을 만든 방식이 이거였다 — 값싼 모델 여러 개를 하나하나 짜맞춰 하나의 프로덕트로 세우는 것.

이게 왜 중요하냐면, **모델 자체의 지능 못지않게 '패키징 능력'이 경쟁력이 되기 때문**이다. 같은 원료를 쥐고도 누구는 소매 완제품을 뽑고 누구는 못 뽑는다. 그 차이가 하네스 설계, 즉 모델을 감싸는 시스템의 정교함에서 갈린다. (이 주제는 예전에 [하네스 엔지니어링 시대](/posts/harness-engineering-era/)에서 따로 정리해 둔 적이 있다.) 그리고 개인적으로는, 앤트로픽/클로드가 바로 이 '패키징'을 유난히 잘한다고 느낀다. 원료(모델)도 세지만, 그걸 도구·컨텍스트·에이전트로 감싸 완제품처럼 쓰게 만드는 솜씨가 남다르다.

정리하면, 앞으로 나올 서비스와 프로덕트는 두 갈래일 것 같다. **(1) 최선단 모델의 소매 지능을 그대로 얹은 것**, 그리고 **(2) 저가 원료 + 뛰어난 패키징으로 소매 완제품을 조립한 것.** 둘 다 시장이 있다.
...
**Q. 프롬프트 한 번에 프로덕트가 나온다는데, 그럼 개발자는 필요 없어지나?**

그렇게 읽지는 않는다. 작업의 위임 단위가 커졌을 뿐, 사람의 자리는 '무엇을 만들지 정의하고, 나온 결과를 판별·교정하는 쪽'으로 올라간다. 한 번에 나온 결과물의 퀄리티가 기대치에 못 미친다는 점이 오히려 그 자리의 중요성을 보여준다. 다만 '코드 한 줄'을 파는 일의 가치는 빠르게 줄어들 것 같다.

**Q. 그러면 비싼 최선단 모델만 쓰면 되는 것 아닌가?**

효율로 보면 아니다. 단순·반복·대량 작업엔 도매 원료(저가 모델)가 훨씬 낫다. 100만 토큰에 $50 vs 몇 센트의 차이는 규모가 커지면 감당이 안 된다. 핵심은 '어떤 일에 어떤 티어의 지능을 붙이느냐'다. 슈퍼맨을 편의점 계산대에 세울 필요는 없다.

**Q. 저가 모델 + 좋은 패키징이 최선단 모델을 대체할 수 있나?**

특정 과제에선 충분히 가능하다고 본다. 다만 '아주 어렵고 긴 호흡의 일'에서는 원료 자체의 지능이 병목이 된다. 하네스로 메울 수 있는 격차와 못 메우는 격차가 따로 있다. 그 경계가 어디냐가 앞으로의 관전 포인트다.
...
그 반대편엔 100만 토큰에 몇 센트짜리 도매 원료가 있다. 완제품을 비싸게 사거나, 원료를 싸게 사서 직접 조립하거나 — 지능의 시장이 도매와 소매로 갈라지고 있다. 그리고 이 분화야말로 AI가 초기 시장을 벗어나 진짜 확장기에 들어섰다는 신호다.
