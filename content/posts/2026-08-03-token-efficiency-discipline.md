...
공급망 상단도 같은 전제 위에서 움직인다. 2026년 7월 이재용 삼성전자 회장이 샌프란시스코 OpenAI 본사에서 샘 올트먼을 만나 AI·반도체 협력을 논의했고, 관측의 초점은 HBM과 파운드리에 맞춰졌다([The Korea Pulse](https://pulse.koreasignals.com/posts/lee-jae-yong-meets-sam-altman-as-samsung-eyes-the-ai-chip-race/)). 확정된 계약이 아니라 아직 기대의 영역이지만, 추론 단가가 내려갈수록 그 추론을 떠받칠 메모리와 제조 능력을 미리 묶어두려 한다는 신호다.
...
① 초기화 에이전트는 딱 한 번만 돌면서 작업 환경과 규칙, 진행 상황을 외부 저장소에 기록한다. 매 세션이 되풀이할 필요가 없는 준비 작업을 여기서 끝내는 역할이다.

② 이후의 실행 세션들은 저장소를 통째로 컨텍스트에 싣지 않는다. 자기 작업에 필요한 조각만 골라 읽어 온다.

③ 작업을 마치면 결과와 진행 상황을 저장소에 다시 남긴다. 다음 세션은 그 지점부터 이어받으므로, 세션이 바뀌어도 맥락이 끊기지 않는다.

가운데 저장소가 이 구조의 핵심이다. 기억이 컨텍스트가 아니라 디스크에 놓이기 때문에, 작업이 아무리 길어져도 활성 컨텍스트는 얇게 유지된다. 매 호출에 실리는 토큰과 비용이 함께 줄어드는 것은 그 결과다.
...
시장도 이미 그 전제를 가격에 넣고 있다. 2026년 7월 31일 SK하이닉스는 장중 29.95% 오른 171만 8,000원으로 상한가에 닿았고 삼성전자도 20% 넘게 뛰었는데, 그 수준에서도 두 종목은 증권사 목표주가의 절반 안팎에 머물렀다([The Korea Pulse](https://pulse.koreasignals.com/posts/sk-hynix-touches-koreas-daily-limit-at-1718000-won-and-analysts-still-see-room/)).
